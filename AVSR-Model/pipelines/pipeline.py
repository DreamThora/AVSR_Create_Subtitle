...
class InferencePipeline(torch.nn.Module):
    ...
    def process_landmarks(self, data_filename, landmarks_filename, i= None):
        ...
        if self.modality in ["video", "audiovisual"]:
            if isinstance(landmarks_filename, str):
                ...
                landmarks = self.landmarks_detector(data_filename, i)
            return landmarks


    def forward(self, data_filename, landmarks_filename=None, i = None):
        if i is not None:
        # Do something with the value of `i`
            print(f"Forward receive i: {i}")
        # assert os.path.isfile(data_filename), f"data_filename: {data_filename} does not exist."
        landmarks = self.process_landmarks(data_filename, landmarks_filename, i)
        data = self.dataloader.load_data(data_filename, landmarks, i=i)
